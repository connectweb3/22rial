...
function parseCSV(csv) {
    ...
    const requiredHeaders = ['id', 'name', 'description', 'image'];
    ...
    // Identify trait columns (any column that's not a required field)
    const traitColumns = headers.filter(h => !requiredHeaders.includes(h));
...
function previewMetadata() {
    ...
    sample.forEach(nft => {
        ...
        // Create nested structure for preview
        const nestedMetadata = {};
        nestedMetadata[id] = metadata;
        ...
        previewHTML += `
            <div class="metadata-item">
                ...
                <div class="metadata-content">
                    <pre>${JSON.stringify(nestedMetadata, null, 2)}</pre>
                </div>
            </div>
        `;
    });
...
function displayGeneratedMetadata() {
    ...
    let metadataHTML = '';
    keys.forEach(key => {
        // Create the nested structure for display
        const nestedMetadata = {};
        nestedMetadata[key] = generatedMetadata[key];
...
function downloadAllFiles() {
    // Directly download single JSON file with all metadata
    downloadSingleJSON();
}
...
function downloadSingleFile(key) {
    // Create nested structure for individual file download
    const nestedMetadata = {};
    nestedMetadata[key] = generatedMetadata[key];
    
    const blob = new Blob([JSON.stringify(nestedMetadata, null, 2)], { type: 'application/json' });
...
function downloadZipFile() {
    ...
    keys.forEach(key => {
        // Create nested structure for each file in ZIP
        const nestedMetadata = {};
        nestedMetadata[key] = generatedMetadata[key];
        zip.file(`${key}.json`, JSON.stringify(nestedMetadata, null, 2));
    });
...
function downloadCSVTemplate() {
    const template = `ID,name,description,image,trait1,trait2
companion1,companion1,Spyd Corp Companion. Become one of us.,ipfs://QmeA7r3SSgyEPzUvtJHfSekGZStK9d33CaCiRBQY2HZEJF,value1,value2
companion2,companion2,Spyd Corp Companion. Become one of us.,ipfs://QmQqiae7KHme7fVJf6PQciPvf12pUeWkn61y3D5jg7Mk3U,value3,value4`;
